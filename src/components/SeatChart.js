...
const SeatChart = ({ occasion, bookMySeat, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await bookMySeat.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await bookMySeat.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
